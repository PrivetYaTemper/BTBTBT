...
function theme_options_add_page() {
	add_theme_page( __( 'Настройки темы' ), __( 'Настройки темы' ), 'edit_theme_options', 'theme_options', 'theme_options_do_page' );
}
...
$griddesign = array(
	'one' => array(
		'value' => 'one',
		'label' => __( 'Показывать 1 изображение записи' ),
		'checked' => 'checked'
	),
	'all' => array(
		'value' => 'all',
		'label' => __( 'Показывать все изображения записи' ),
		'checked' => ''
	)
);
...
function theme_options_do_page() {
	...
	?>
	<div class="wrap">
		<?php screen_icon(); echo "<h2>" . __( 'Настройки темы ' ) . get_current_theme() . "</h2>"; ?>
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				 ...
				 */
				?>
				<tr valign="top"><th scope="row"><?php _e( 'Отображение сетки' ); ?></th>
					<td>
						<fieldset><legend class="screen-reader-text"><span><?php _e( 'Отображение сетки' ); ?></span></legend>
						<?php
							if ( ! isset( $checked ) )
								$checked = '';
				 ...
				 */
				?>
				<tr valign="top"><th scope="row"><?php _e( 'Рубрика блога' ); ?></th>
					<td>
						...
						<label class="description" for="mansion_theme_options[blog]"><?php _e( 'Применяется только в случае, если вы создаете Страницу на блоге с помощью Шаблона Страницы' ); ?></label>
					</td>
				</tr>
